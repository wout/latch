...
describe Latch::StoredFile do
  memory_store = Latch::Storage::Memory.new(base_url: "https://example.com")
  ...
  describe "#extension" do
    ...
    it "falls back to filename metadata" do
      file = TestUploader::StoredFile.new(
        id: "abc123",
        storage_key: "store",
        metadata: Latch::MetadataHash{"filename" => "photo.png"}
      )
  ...
  describe "#size" do
    it "returns Int64 from integer metadata" do
      file = TestUploader::StoredFile.new(
        id: "file.jpg",
        storage_key: "store",
        metadata: Latch::MetadataHash{"size" => 1024_i64}
      )
    ...
    it "coerces Int32 to Int64" do
      file = TestUploader::StoredFile.new(
        id: "file.jpg",
        storage_key: "store",
        metadata: Latch::MetadataHash{"size" => 512_i32}
      )
  ...
  describe "#variant_location" do
    it "derives a variant path with extension" do
      file = TestUploader::StoredFile.new(id: "uploads/abc123.jpg", storage_key: "store")

      file.variant_location("sizes_large").should eq("uploads/abc123/sizes_large.jpg")
    end

    it "derives a variant path without extension" do
      file = TestUploader::StoredFile.new(
        id: "uploads/abc123",
        storage_key: "store",
      ...
      file.variant_location("sizes_large").should eq("uploads/abc123/sizes_large")
    end

    it "preserves the full directory structure" do
      file = TestUploader::StoredFile.new(id: "a/b/c/file.png", storage_key: "store")

      file.variant_location("thumb").should eq("a/b/c/file/thumb.png")
    end
  end
end
