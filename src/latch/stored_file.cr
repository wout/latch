...
abstract class Latch::StoredFile
  ...
  # Returns the location for a named variant of this file. The variant is
  # stored as a sibling under a subdirectory matching the original basename.
  #
  # ```
  ...
  def download(**options) : File
    ...
    tempfile = File.tempfile("latch", suffix)
